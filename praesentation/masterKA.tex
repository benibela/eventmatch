...
\put(25,200){\circle{120}\circle{70}\circle{40}}
\put(15,195){\Huge \Mobilefone}
\put(175,200){\circle{120}\circle{70}\circle{40}}
\put(165,195){{\Huge \Mobilefone}}
\put(90,95){{\Huge \Mobilefone}}
\put(100,100){\circle{170}\circle{120}\circle{70}\circle{40}}
...
\begin{frame}
	\animate<1-10>
	\frametitle{Nachrichten werden gelogt} %Wie berechnet man aus aufgezeichneten Ereignissen welche Nachricht empfangen wurde?\mybreak 
...
%	\end{tabular}
	...
	\onslide<1->Führen zu folgenden Protokollen:\mybreak
	\begin{center}
...
\vspace{1em}
\begin{overprint}
\onslide<1-2> \begin{center}$L_0 = \{A, B, C, D, E, F\}$\end{center}
\onslide<3-4> \begin{center}$L_0 = \{A, B, C, D, E, F, \quad \color<3>{red}{A, B, B, C,C,C,D,D,E}\}$\end{center}
\onslide<5-6> \begin{center}$L_0 = \{A, B, C, D, E, F, {\color<5>{red}X, X}, A, B, B, C,C,C,D,D,E,{\color<5>{red} X, X, X, X}\}$\end{center}
\end{overprint}
\vspace{1em}
...
\uncover<2-6>{\begin{overprint}
%\only<1,3>{\phantom{Problem:} x \newline \phantom{in der oper} s}
\onslide<2>\color{red}Problem: Zu wenig Sender! \newline \phantom{Problem:} uSRC benötigt eindeutige Sender
\onslide<4>\color{red}Problem: Überlappungen! 
\onslide<6>\color{orange}uSRC ist NP-vollständig!
\end{overprint}}
\end{frame}
...
\only<1>{	
Sendelog:
\[L_0 = \{A, B, C, D, E, F, X, X, A, B, B, C,C,C,D,D,E,  X, X, X, X\}\]}
...
%	\end{tabular}}
  ...
  \begin{overprint}
	\onslide<3>\color{red}Problem: Können die Sendeereignisse  zugewiesen werden?
...
\only<5>{
	...
	\phantom{Dasselbe für links $\Rightarrow$ Algorithmus für u}SRC in $O(2^l n^l)$
	}
\end{frame}
